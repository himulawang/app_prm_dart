...
class IRedis extends IRedisCommand {
  ...
  Future connect() {
    ...
    */
    ...
    .then((info) {
      ...
      return chrome.sockets.tcp.connect(_socketId, host, port);
    })
    .then((resultCode) {
      print('connected');
      if (resultCode != 0) throw new IRedisException('Cannot connect host.');
    })
    .then(_onConnect)
    .then(_doAuth)
    .then(_selectDB)
  ...
  Future close() {
    ...
    return chrome.sockets.tcp.close(_socketId).then((_) => status = STATUS_DISCONNECTED);
  }
  ...
  Future _send(List cmd) {
    if (MODE_TRANSACTION) {
      ...
      write(iRESPEncoder.convert(cmd));
    }
  ...
  write(List list) {
    var bytes  = new chrome.ArrayBuffer.fromBytes(list);
    print(bytes);
    chrome.sockets.tcp.send(_socketId, bytes).then((sendInfo) {
      print(sendInfo.resultCode);
      print(sendInfo.bytesSent);
    });
  }
  ...
  _onConnect(_) {
    chrome.sockets.tcp.onReceive.listen(_onData, onError: (e) {
  ...
  _onData(chrome.ReceiveInfo info) {
    if (info.socketId != _socketId) return;
    iRESPDecoder.convert(info.data.getBytes());
  }
  ...
  dynamic _onPubSubMessage(List result) {
  ...
  Future<String> subscribe(String channel, Function onData,
    ...
    write(iRESPEncoder.convert([CMD_SUBSCRIBE, channel]));
  ...
  Future<String> unsubscribe(String channel) {
    ...
    write(iRESPEncoder.convert([CMD_UNSUBSCRIBE, channel]));
  ...
  Future<String> psubscribe(String pattern, Function onData,
    ...
    write(iRESPEncoder.convert([CMD_PSUBSCRIBE, pattern]));
  ...
  Future<String> punsubscribe(String pattern) {
    ...
    write(iRESPEncoder.convert([CMD_PUNSUBSCRIBE, pattern]));
