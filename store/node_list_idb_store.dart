...
class NodeListIndexedDBStore extends IIndexedDBStore {
  static const String OBJECT_STORE_PK_NAME = '_pk';
  static const String OBJECT_STORE_INDEX_NAME = '_index';

  static const Map store = const {"type":"indexedDB","master":"prm","objectStore":"NodeList"};

  static Future<NodeList> set(NodeList list) {
    if (list is! NodeList) throw new IStoreException(22013);
    ...
    List waitList = [];
    list.getToAddList().forEach((String childId, Node model) {
      waitList.add(_addChild(model, handler));
    });

    list.getToSetList().forEach((String childId, Node model) {
      waitList.add(_setChild(model, handler));
    });

    list.getToDelList().forEach((String childId, Node model) {
  ...
  static Future del(NodeList list) {
    ObjectStore handler = new IIndexedDBHandlerPool().getReaderHandler(store);

    List waitList = [];
    list.getList().forEach((String childId, Node model) {
      waitList.add(_delChild(model, handler));
    });
  ...
  static Future _addChild(Node model, ObjectStore handler) {
  ...
  static Future _setChild(Node model, ObjectStore handler) {
    // indexedDB do not like redis, put(set) will overwrite the whole key
    // so we use toSet filter the whole _args
    Map toSetAbb = {};
    Node._mapAbb.forEach((abb, i) {
      if (Node._columns[i]['toSet']) return;
  ...
  static Future _delChild(Node model, ObjectStore handler) {
